...
export function ClientModal({ isOpen, onClose, isEditMode = false, clientData, onSave }: ClientModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se for um novo cliente, mostrar modal de confirmação de contrato
      if (!isEditMode && responseData.id) {
        setNewClientId(responseData.id);
        setNewClientName(formData.nome);
  ...
  // Função para fechar o modal de confirmação de contrato
